...
rgb_to_ycbcr(UINT8 *rgb_unit, ycbcr_unit *ycc_unit, int x, int w) {
    ...
    UINT8 r, g, b;
    int src_pos = (x + w * (DCTSIZE - 1)) * 3; // 自底部向上变换以倒置图片
    int dst_pos = 0;
    int i, j;
    for (j = 0; j < DCTSIZE; j++) {
        for (i = 0; i < DCTSIZE; i++) {
        ...
        src_pos = src_pos - 3 * DCTSIZE - 3 * w; // 行坐标不断减小
    }
}
...
set_bits(BITS *bits, INT16 data) {
    UINT16 temp;
    if (data < 0) temp = ~data + 1;
    else temp = data;
    int i;
    for (i = 15; i >= 0; i--)
        if ((temp & (1 << i)) != 0) break;
    bits->len = i + 1;
    if (data < 0) bits->val = data + (1 << (i + 1)) - 1; // 若为负数则在原数基础上加2^x-1进行映射
    else bits->val = data; // 若为正数则不变
}
...
jpeg_encode(compress_io *cio, bmp_info *binfo) {
    ...
    int in_size = in->end - in->set; // bmp图片每行大小
    int offset = binfo->offset + (binfo->datasize / in_size - 1) * in_size; // 将偏移量设置到bmp图最后一行以保证图片倒置
    fseek(in->fp, offset, SEEK_SET);

    for (y = 0; y < binfo->height; y += 8) {
        in->flush_buffer(cio);
        for (x = 0; x < binfo->width; x += 8) {
            rgb_to_ycbcr(in->set, &ycc_unit, x, binfo->width); // 将rgb图转为ycbcr图
            jpeg_fdct(ycc_unit.y);
            jpeg_fdct(ycc_unit.cb);
            jpeg_fdct(ycc_unit.cr); // 分别对三通道进行fdct变换
            jpeg_quant(&ycc_unit, &q_unit); // 存储量化表
            ...
            jpeg_compress(cio, q_unit.cr, &dc_cr, h_tables.ch_dc, h_tables.ch_ac); // 分别对三通道进行Huffman编码
        }
    }
